...
def _extrair_colaboradores_projetos(soup, nome_do_pesquisador):
    ...
    if not ancora_projetos:
        return []
    container_de_dados = ancora_projetos.find_parent('div', class_='title-wrapper').find_next('div', class_='layout-cell-12')
...
def _extrair_dados_basicos(soup):
    ...
    try:
        ...
        resumo = re.sub(r'\s*\((?:Texto informado pelo autor)\)\s*$', '', resumo_tag.get_text(strip=True), flags=re.IGNORECASE)
    except AttributeError:
        resumo = ""
...
def _extrair_formacao(soup):
    ...
    if not container_div:
        return formacoes
    itens_periodo = container_div.find_all('div', class_='layout-cell-3', recursive=False)
    for div_periodo in itens_periodo:
        ...
        if "Doutorado" in descricao_completa: tipo = "Doutorado"
        elif "Mestrado" in descricao_completa: tipo = "Mestrado"
        elif "Especialização" in descricao_completa: tipo = "Especialização"
        elif "Graduação" in descricao_completa: tipo = "Graduação"
        elif "Pós-Doutorado" in descricao_completa or "Pós-doutorado" in descricao_completa: tipo = "Pós-doutorado"
        elif "Aperfeiçoamento" in descricao_completa: tipo = "Aperfeiçoamento"
        elif "Curso técnico/profissionalizante" in descricao_completa: tipo = "Técnico/Profissionalizante"
        dados_da_formacao = {
...
def _extrair_projetos(soup):
    ...
    for div_periodo, div_info in zip(divs_periodo, divs_descricao):
        ...
        titulo_do_projeto = linhas_de_info[0] if linhas_de_info else "Título não encontrado"
        if titulo_do_projeto.startswith("Periódico:") or titulo_do_projeto.startswith("Grande área:"):
            continue
        ...
        dados_do_projeto = {
            ...
            "situacao_projeto": situacao_match.group(1).strip() if situacao_match else None,
            "natureza_projeto": natureza_match.group(1).strip() if natureza_match else None,
        }
...
def extrair_dados_professor(conteudo_html):
    ...
    try:
        ...
        dados_extraidos['coautores_publicacoes'] = _extrair_coautores(soup, nomes_de_citacao)
        dados_extraidos['colaboradores_projetos'] = _extrair_colaboradores_projetos(soup, nome_do_pesquisador)
...
def consolidar_dados_professor_em_linha_unica(dados_extraidos):
    ...
    linha_unica['coautores_publicacoes'] = "; ".join(dados_extraidos.get('coautores_publicacoes', []))
    linha_unica['colaboradores_projetos'] = "; ".join(dados_extraidos.get('colaboradores_projetos', []))
    return linha_unica
...
def processar_diretorio(diretorio_de_entrada):
    ...
    print(f"Iniciando o processamento de {len(arquivos_html)} arquivos HTML de '{diretorio_de_entrada}'...")
    dados_consolidados = []
    for i, nome_arquivo in enumerate(arquivos_html):
        ...
        if dados_extraidos and dados_extraidos.get('nome'):
            ...
            print(f"    -> Aviso: Nenhum dado extraído de {nome_arquivo}. O arquivo pode estar vazio ou mal formatado.")
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Diretório contendo os arquivos HTML de entrada.")
    parser.add_argument("--output", required=True, help="Caminho do arquivo de saída. Ex: 'dados.csv' ou 'dados.json'.")
    ...
    try:
        if extensao == '.json':
            ...
            with open(caminho_de_saida, 'w', encoding='utf-8') as arquivo_json:
                json.dump(dados_em_formato_json, arquivo_json, ensure_ascii=False, indent=4)
        elif extensao == '.csv':
